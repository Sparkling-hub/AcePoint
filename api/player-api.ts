...
const fetchData = async (collectionName: string, queryField: string | null = null, queryValue: string | null = null) => {
    let data;
...
const favouriteCoach = async (coachRef: any) => {
    try {
        ...
        await runTransaction(db, async (transaction) => {
            transaction.update(playerRef, { favoriteCoach: [...updatedFavoriteCoaches, coachRef] });
            transaction.update(doc(db, 'coach', coachRef), { followedPlayer: [...updatedCoachList, currentUser.uid] });
        });
...
const unfavoriteCoach = async (coachRef: any) => {
    try {
        ...
        await runTransaction(db, async (transaction) => {
            transaction.update(playerRef, { favoriteCoach: updatedFavoriteCoaches.filter(coach => coach !== coachRef) });
            transaction.update(doc(db, 'coach', coachRef), { followedPlayer: updatedCoachList.filter(player => player !== currentUser.uid) });
        });
...
export { findByName, favouriteCoach, findCoachByName, unfavoriteCoach, favoriteCoachList,fetchData };
