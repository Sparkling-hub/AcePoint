...
describe('GoogleAuthIOS', () => {
    ...
    it('stores user data in AsyncStorage on successful authentication', async () => {
        ...
        await waitFor(() => {
            expect(ReactNativeAsyncStorage.setItem).toHaveBeenCalledWith('username', 'Test User');
            expect(ReactNativeAsyncStorage.setItem).toHaveBeenCalledWith('email', 'test@example.com');
            expect(ReactNativeAsyncStorage.setItem).toHaveBeenCalledWith('image', 'http://example.com/photo.jpg');
        });
    });
